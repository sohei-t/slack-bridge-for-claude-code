...
# --- Content Scout 提案ハンドラー登録 ---
try:
    sys.path.insert(0, str(Path.home() / "Desktop/AI-Apps/content-scout-agent"))
    from project.src.proposal_handlers import register_handlers
    register_handlers(app)
    log.info("Content Scout 提案ハンドラー登録完了")
except ImportError as e:
    log.warning(f"Content Scout ハンドラー未登録（モジュール未検出）: {e}")
