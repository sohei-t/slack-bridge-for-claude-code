...
# Sends a Bot DM with tmux pane capture showing the final output.
#
# Hook type: Stop
...
# Build message
parts = [":white_check_mark: *Claude Code completed*"]
if dir_name:
    parts.append(f":file_folder: {dir_name}")
parts.append(f":clock3: {time_str}")
if pane_content:
    parts.append("")
    parts.append(f"```\n{pane_content}\n```")
